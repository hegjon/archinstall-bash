...
luks_uuid() {
  cryptsetup luksUUID "$1" 2>/dev/null || die "Unable to get UUID for Luks device: $1"
}

# Luks2.create_keyfile(): /etc/cryptsetup-keys.d/<mapper>.key in the target,
# added as key slot 1 and referenced from /etc/crypttab.
luks_create_keyfile() {
  local dev=$1 mapper=$2 password=$3 target=$4 override=${5:-}
  local kf_path="/etc/cryptsetup-keys.d/$mapper.key"
  local key_file="$target$kf_path"
  if [[ -e $key_file ]]; then
    if [[ -z $override ]]; then
      info "Key file $key_file already exists, keeping existing"
      return 0
    fi
    info "Key file $key_file already exists, overriding"
  fi
  mkdir -p "${key_file%/*}"
  generate_password 512 >"$key_file"
  chmod 0400 "$key_file"
  debug "Adding additional key-file $key_file"
  sys_cmd_input "$password" cryptsetup -q -v --key-file - luksAddKey "$dev" "$key_file" ||
    die "Could not add encryption key $key_file to $dev: $SYS_CMD_OUTPUT"
  luks_crypttab_entry "$dev" "$mapper" "$target" "$kf_path" 'luks,key-slot=1'
}

# Luks2.create_crypttab_entry(): passphrase prompt at boot.
luks_create_crypttab_entry() {
  local dev=$1 mapper=$2 target=$3
  mkdir -p "$target/etc"
  luks_crypttab_entry "$dev" "$mapper" "$target" none luks
}

luks_crypttab_entry() {
  local dev=$1 mapper=$2 target=$3 key_file=$4 options=$5 uuid
  uuid=$(luks_uuid "$dev")
  debug "Adding crypttab entry for key $key_file"
  printf '%s UUID=%s %s %s\n' "$mapper" "$uuid" "$key_file" "$options" >>"$target/etc/crypttab"
}
